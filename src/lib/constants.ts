export const TECH_STACKS = [
  ...
  { name: "Node.js", color: "bg-emerald-100 text-emerald-800 border-emerald-200" },
...
export function getTechColor(tech: string): string {
  const found = TECH_STACKS.find(t => t.name.toLowerCase() === tech.toLowerCase());
  return found?.color || "bg-secondary text-secondary-foreground border-border";
}

export const JOB_LEVELS = ["Junior", "Mid", "Senior", "Lead", "Manager"] as const;
